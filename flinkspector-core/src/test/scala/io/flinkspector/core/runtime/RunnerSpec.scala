...
class RunnerSpec extends CoreSpec {
  ...
  trait RunnerCase {
    ...
    val cluster = mock[MiniCluster]

    when(cluster.listJobs())
      .thenReturn(CompletableFuture.completedFuture[java.util.Collection[JobStatusMessage]](List.empty[JobStatusMessage]))
