...
import org.apache.flink.streaming.test.input.{EventTimeInputBuilder, InputBuilder}
...
class StreamTestEnvironmentSpec extends CoreSpec with Eventually {
  ...
  "The environment" should "initialize" in {
    StreamTestEnvironment.createTestEnvironment(1)
  }
  ...
  it should "trigger a failed time-out" in {
    val sadVerifier = new SimpleOutputVerifier[Integer] {
      override def verify(output: util.List[Integer]): Unit = {
        throw new StreamTestFailedException(new AssertionError())
      }
    }
  ...
  it should "handle more than one sink" in {
    val env = StreamTestEnvironment.createTestEnvironment(1)
    val list = List[Integer](1,2,3,4,5,6,7,8)
    val split = env.fromElements(list: _*)
      .split(new OutputSelector[Integer] {
        override def select(number: Integer): Iterable[String] = {
