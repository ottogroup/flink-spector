...
class ProductMatcher[T <: Product,V](key: String,
  ...
  def resolve(key: String, product: Product): V = {
    product.productElement(mask.indexOf(key))
  }
...
class DataStreamTest extends CoreSpec {
  ...
  "assert test" should "do shi" in {
    ...
    Foo.getClass.getMethods.find(x => x.getName == "apply" && x.isBridge).get.invoke(Foo, params map (_.asInstanceOf[AnyRef]): _*).asInstanceOf[Foo]
